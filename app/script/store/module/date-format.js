...
(function (root, factory) {
	...
	var TimeFormat = function() {
		...
		this.countdown = function(msel,sign) {
			...
			if (msel > 0) {
				msel = msel / 1000;
				dd = parseInt(msel / 3600 / 24);
				hh = parseInt((msel - dd*3600*24)/3600);
				mm = parseInt((msel - dd*3600*24-hh*3600)/60);
				ss = parseInt(msel % 60);
			} else {
				dd=hh = mm = ss = 0;
			}
			...
			if (ss < 10) ss = "0" + ss;
			if(sign){
				return dd + hh + sign + mm + sign + ss;
			}else{
				return dd + '<span>天</span>' + hh + "<span>时</span>" + mm + "<span>分</span>" + ss+ "<span>秒后开始</span>";
			}
		};
		/*
		 * 多久前
