...
define(function (require, exports, module) {
    ...
    //--------------------------------------------------【转场nav样式选中】
    Mobilebone.callback = function (page_in, page_out) {   //mobilebone的回调方中页面参数作为容器去获取元素
    ...
    Mobilebone.mergeCallback = true; //全局的回调方法和每个页面上自定义的回调方法是合并还是覆盖。默认true表示合并。

    var popAlert=new Dialog('alert');//alert弹窗
    //--------------------------------------------------【路由】
    Mobilebone.classAnimation = "fade";   //页面的动画过场默认是左右切换,fade-->淡入淡出
    Mobilebone.rootTransition = {       //Mobilebone暴露出来的一个可以修改回调对象的接口Mobilebone.rootTransition
            ...
            base.ajax({
                ...
                success: function (data) {
                    ...
                    Handlebars.registerHelper("helper-proMark",function(){
                        ...
                        if(this.status==10){
                            return new Handlebars.SafeString('<span>预告</span><i class="icon icon-pro-mark orange"></i>');
                        }
                        if(this.isNewHand==1){
